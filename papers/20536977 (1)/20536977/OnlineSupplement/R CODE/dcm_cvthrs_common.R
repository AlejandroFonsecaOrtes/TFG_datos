...
library(CVTuningCov)
# Y: array, n*m_max*p, X: time points, vector of length m
cvthreshold <- function(Y, X, h, Xs, threshold.min = 0.1, threshold.max = 0.6) {
  
  nn = dim(Y)[1]; pp = dim(Y)[2]; m = dim(Y)[3]
  #############################################
  ...
  for (shu in 1:ns) {
    ...
    for (thresh in 1:length(thresholding)) {
      ...
      for (j in 1:N1) {
        ...
        a = subj_in[j,]
        res = (1:nn)[is.na(pmatch(1:nn, a))]
        sigma.NW.jihe1 = dcm_smoothCov(Y[a,,], X, h, x)
        sigma.NW.jihe.soft1 = sign(sigma.NW.jihe1) * (abs(sigma.NW.jihe1) - s) *
          ((abs(sigma.NW.jihe1) - s) > 0)

        sigma.NW.jihe.res1 = dcm_smoothCov(Y[res,,], X, h, x)
